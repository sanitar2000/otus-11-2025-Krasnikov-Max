...
class GpbLotsRegistryPage {
  constructor(page) {
    this.page = page;
    this.gpbMenuButton = page.getByRole('button', { name: 'Управление партиями продуктов переработки зерна', exact: true });
    this.registryLink = page.locator('a.sidebar-menu__link:has-text("Реестр партий продуктов переработки зерна")');
  ...
  async openGpbRegistry() {
    await this.gpbMenuButton.click();
    await expect(this.registryLink).toBeVisible();
    await this.registryLink.click();
    await expect(this.page).toHaveURL(/\/lots\/gpb\/list/);
  }
  ...
  async selectRandomLotWithCurrentMassGreaterThan(minMass = 10) {
...
module.exports = { GpbLotsRegistryPage };