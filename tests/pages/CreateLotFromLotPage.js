...
class CreateLotFromLotPage {
  constructor(page) {
    this.page = page;
    this.moreButton = page.locator('button:has-text("Еще")');
    this.menuItem = page.locator('div[role="menuitem"]:has-text("Сформировать партию из партии")');
    this.formTitle = page.locator('span.title:has-text("Формирование партии зерна из других партий")');
    ...
    this.statusChip = page.locator('.v-chip__content:has-text("Создано")');
  }
  ...
  async waitForCreatedStatus() {
    await expect(this.statusChip).toBeVisible({ timeout: 15000 });
  }
}

module.exports = { CreateLotFromLotPage };